...
<font color=#FF6600>
**Important**: `iNEXT` has been updated in 2021 March since the confidence interval of coverage-based R/E  was modified. Please users updated package `iNEXT` from CRAN again.
</font>
...
NOTE: Only show five estimates, call iNEXT.object$iNextEst$size_based. to show complete output.
```

The second data frame of list `$iNextEst` (as shown below for 'coverage_based') includes the reference sample coverage ('goalSC'), real sample coverage ('SC'), sample size (`m`, i.e., each of the 40 knots), the method (`interpolated`, `observed`, or `extrapolated`, depending on whether the size `m` is less than, equal to, or greater than the reference sample size), the diversity order, the diversity estimate of order q (`qD`) conditioning on 'goalSC', the 95% lower and upper confidence limits of diversity (`qD.LCL`, `qD.UCL`). These sample coverage estimates with confidence intervals are used for plotting the coverage-based R/E curves. 

```{r eval=FALSE}
head(out$iNextEst$coverage_based)
```

```
$coverage_based
   Assemblage goalSC    SC   m        Method order.q     qD qD.LCL qD.UCL
1  Girdled     0.122 0.122   1   Rarefaction       0  1.000  1.000  1.000
10 Girdled     0.900 0.900  84   Rarefaction       0 18.912 15.902 21.923
20 Girdled     0.929 0.929 168      Observed       0 26.000 21.492 30.508
30 Girdled     0.948 0.948 248 Extrapolation       0 30.883 25.149 36.618
40 Girdled     0.964 0.964 336 Extrapolation       0 34.731 27.187 42.275
NOTE: Only show five estimates, call iNEXT.object$iNextEst$coverage_based. to show complete output.
```
...
The argument `facet.var="Order.q"` and `color.var="Assemblage"` creates a separate plot for each diversity order site, and within each plot, different colors are used for two 
sites. 
...
The following commands return the coverage‐based R/E sampling curves in which different colors are used for the two sites (`facet.var="Assemblage"`) and for three orders 
(`facet.var="Order.q"`)
